...
import com.machine.screensaverdemo.databinding.DialogItemBinding

class ItemDialog(
    val arItem: ArItem,
    val itemPosition: Int,
    val itemDetailsInterface: ItemDetailsInterface
) :
    DialogFragment() {
    ...
    companion object {
        fun getInstance(
            arItem: ArItem,
            itemPosition: Int,
            itemDetailsInterface: ItemDetailsInterface
        ) =
...
//            binding?.button3?.playAnimation()
    ...
    private fun setupDetails() {
        arItem.apply {
            binding?.detailsImage?.setImageDrawable(this.image)
            binding?.detailsName?.text = this?.itemName
            binding?.detailsPrice?.text = "$" + this?.itemPrice.toBigDecimal().setScale(2)
